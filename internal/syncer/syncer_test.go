...
// caller invokes the returned cleanup func — this directory was once never
// removed, leaking one temp dir per bare-dir sync.
func TestSynthesizeCleansUpTempDir(t *testing.T) {
	...
	if _, err := os.Stat(tmp); err != nil {
		t.Fatalf("synthesized dir must exist before cleanup: %v", err)
	}
	cleanup()
	if _, err := os.Stat(tmp); !os.IsNotExist(err) {
		t.Fatalf("synthesized dir %s must be removed after cleanup, stat err = %v", tmp, err)
	}
}

func TestSyncRejectsEmptyDir(t *testing.T) {
	_, _, err := loadOrSynthesize(t.TempDir())
	var de *diag.Error
...
// synthesis is strictly a fallback for dirs that opted out of pack.cue. The
// real-pack path's cleanup must be a no-op: it must never remove dir itself.
...
`), 0o644)
	...
	if _, err := os.Stat(dir); err != nil {
		t.Fatalf("real-pack cleanup must not remove the caller's dir: %v", err)
	}
}
