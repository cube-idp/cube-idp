...
func desiredState(ctx context.Context, cube *config.Cube, eng engine.Engine) (desired, orphanOnly []*unstructured.Unstructured, entries []lock.Entry, err error) {
	...
	// but packDeps is kept (DEP3 threads it into the Deliver calls below,
	// exactly as up.Run's deliver pass will); a graph error surfaces from
	// diff exactly as CUBE-4016/4017 already do above, before any object is
	// added to desired/orphanOnly.
