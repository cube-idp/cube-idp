...
func PortForward(ctx context.Context, cfg *rest.Config) (string, func(), error) {
	...
	if err != nil {
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "cannot build Kubernetes client for port-forward",
	...
	if err != nil {
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "cannot list zot pods", "check cluster connectivity")
	}
	if len(pods.Items) == 0 {
		return "", nil, diag.New(diag.CodePortForwardFail, "no running zot pod to port-forward to",
	...
	if err != nil {
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "cannot build SPDY transport for port-forward",
			"check kubeconfig and cluster connectivity")
	}
	...
	if err != nil {
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "cannot create port-forwarder to zot",
			"retry `cube-idp up`")
	}
	...
	case err := <-errCh:
		close(stopCh)
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "port-forward to zot failed before becoming ready",
			"re-run `cube-idp up`; check `kubectl -n cube-idp-system get pods`")
	case <-ctx.Done():
		close(stopCh)
		return "", nil, diag.Wrap(ctx.Err(), diag.CodePortForwardFail, "port-forward to zot canceled before becoming ready",
			"re-run `cube-idp up`; check `kubectl -n cube-idp-system get pods`")
	case <-time.After(readyDeadline):
		close(stopCh)
		return "", nil, diag.New(diag.CodePortForwardFail, "port-forward to zot did not become ready within 30s",
			"check `kubectl -n cube-idp-system get pods` and node health")
	}
	ports, err := fw.GetPorts()
	if err != nil || len(ports) == 0 {
		close(stopCh)
		return "", nil, diag.Wrap(err, diag.CodePortForwardFail, "port-forward to zot failed", "retry `cube-idp up`")
	}
