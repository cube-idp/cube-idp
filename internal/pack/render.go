...
func (p *Pack) Render(values map[string]any) (*Rendered, error) {
	...
	if err != nil && !os.IsNotExist(err) {
		return nil, diag.Wrap(err, "CUBE-4004", "cannot read pack manifests/ directory", "check directory permissions")
	}
	...
	if _, err := os.Stat(filepath.Join(p.Dir, "chart.yaml")); err == nil {
		...
		return nil, diag.Wrap(err, "CUBE-4004", "cannot read pack chart.yaml", "check file permissions")
	}
