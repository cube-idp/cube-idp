...
- No cube-idp CRDs. The cluster's record of truth is the GitOps engine's own resources plus an SSA inventory.
...
- **Phase 2 (~2-3 wks):** **Argo CD engine implementation + contract suite**
  (D2); `cube-idp trust` + CoreDNS canonical hostname; `diff`; `doctor` with
  CUBE codes; `cube.lock` + `upgrade --plan`; git-ref pack source;
  cnoe-compat loader.
