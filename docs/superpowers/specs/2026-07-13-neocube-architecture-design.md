...
spec:
  ...
  gateway:
    ...
    port: 8443
  packs:
    - ref: oci://ghcr.io/neocube/packs/gitea:1.x     # D9: in default profile —
...
- **Phase 1 — MVP (~3-4 wks):** cobra shell; `cube.yaml` loader + JSON
  ...
  gitea — with gitea in the `neocube init` default profile (D9): admin
...
- **Phase 3:** k3d provider; `vendor` / `up --bundle` air-gap; exec-plugin
  ...
  external-secrets, envoy-gateway); `sync --watch`; `neocube repo create
