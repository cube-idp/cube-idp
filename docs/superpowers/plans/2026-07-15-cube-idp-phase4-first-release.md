...
func TestTrustKeyCanonicalization(t *testing.T) {
	t.Setenv("HOME", t.TempDir()) // isolate the store (RECONCILE: mirror how existing plugin tests isolate os.UserConfigDir — reuse their helper if one exists)
	dir := t.TempDir()
...
- Modify: `internal/pack/kustomize.go`, `internal/pack/render.go`, `internal/pack/render_test.go` (or kustomize_test.go — wherever kustomize render tests live; `grep -rn RenderDir --include="*_test.go" internal/pack`).
...
(NOTE: `--local /repo` — the flag path need not exist; init only joins paths. `RECONCILE:` confirm init doesn't stat the --local path; if it does, use `t.TempDir()`.) RED: the flag doesn't exist.
...
**Collision check (spec §7 risk):** extend the existing network-gated envoy render test (the F8 one that pins CRDs+certgen in the rendered stream — find via `grep -rn "envoy" internal/pack/*_test.go internal/up/*_test.go`) with: no rendered object is a `v1 Service` named `cube-idp-gateway` in namespace `envoy-gateway` (the name must be free for EG's generated Service), and the pack's parsed `GatewayService` matches the EnvoyProxy's `envoyService.name` (read the manifest object). `RECONCILE:` the test's exact file/gating env var — mirror whatever gates that network render test today. Commit `feat(packs/envoy-gateway): stable cube-idp-gateway data-plane service + gatewayService declaration`.
...
// contract: kubeconfig via cluster.New(...).Kubeconfig (or the harness's
// existing conn helper); pod spec: restartPolicy Never, one container
...
```
  ...
  3. **`pr` loop-var rename** (up.go:252): `for i, pr := range refs` → `for i, pref := range refs` (+ the three body uses `pref.Ref`/`pref.Values`); delete the now-moot half of the `pk` comment if it references the shadow.
