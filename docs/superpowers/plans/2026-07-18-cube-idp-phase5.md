...
A11 STATUS: DONE  BRANCH: p5/a11-floci-ui (merged: yes — $PACKS main advanced 278f81f → 96769e2; branch kept, worktree removed)  COMMITS: $PACKS fbfa4ee feat(pack): floci-ui 0.1.0 — web console for the floci local cloud emulator; 96769e2 merge: p5 A11 floci-ui (p5/a11-floci-ui); tag floci-ui/v0.1.0 @ 96769e2; publish run 29663777208 SUCCESS — https://github.com/cube-idp/packs/actions/runs/29663777208; $ROOT ledger only (a01f42e claim + this close)  FINDINGS: AUTHORED-MANIFESTS pack (the floci-family console; sibling of A10/floci). WHY authored: the floci-ui upstream (github.com/floci-io/floci-ui, the floci web console — a React SPA served by a small bundled Node server) is distributed Docker-only and ships NO Kubernetes YAML, so a minimal Deployment + Service was authored per the A11 row's notes (the row IS the spec). SHARED NAMESPACE with A10: floci-ui runs in ns `floci` which the floci pack (A10) already owns/creates (packs/floci/manifests/10-namespace.yaml is on $PACKS main), so THIS pack ships NO Namespace object — it only stamps metadata.namespace: floci on its Deployment/Service/HTTPRoute (flux Kustomization has no targetNamespace — A7/A10 precedent). (1) IMAGE PIN — floci/floci-ui:0.2.0, pinned by tag AND multi-arch index digest sha256:03a261144e0708993c8e48b763a0edb072415feae4325f254beeb1835fa424d9 (registry docker.io/floci/floci-ui). VERIFIED empirically 2026-07-19: `docker buildx imagetools inspect` shows floci/floci-ui:0.2.0 and floci/floci-ui:latest resolve to the SAME index digest → 0.2.0 IS latest stable (the row's 0.2.0 pin is current). Digest recorded as a comment header in manifests/20-floci-ui.yaml and in the README. (2) VERIFY-API — PORTS: the A11 row said "4500 UI / 4501 API"; VERIFIED by RUNNING the image (docker run, throwaway host port 24500→4500, never touching 18443/the conformance leg) that there is NO separate 4501 API port. The image config sets PORT=4500; the container listens on a SINGLE port 4500 (netstat inside → only :::4500 LISTEN; server logs "Started development server: http://localhost:4500"). The server's *code* default is Number(process.env.PORT ?? 4501), but the image OVERRIDES PORT=4500, so nothing ever binds 4501 — the "4501 API" in the row is a misread (UI+API are same-origin on the one PORT). So the pack exposes 4500 only, and sets PORT=4500 explicitly on the Deployment so the listen port can't drift from the 4501 code fallback. Recorded in README ("Ports — one port, 4500") + the 20-floci-ui.yaml header. (3) VERIFY-API — HEALTH PATH: verified by running the image — this SPA server exposes NO dedicated health endpoint (GET /health, /healthz, /readyz, /api/health, … all 404); GET / returns 200 (serves index.html). So the readiness/liveness probes hit GET / on 4500 — the correct, unambiguous readiness signal (A10's "verify the path by running the image, don't guess" precedent; my initial /health guess was wrong and removed). (4) VERIFY-API — ENV NAME (the row's FLOCI_ENDPOINT): CONFIRMED CORRECT. Inspecting the server binary (strings /app/server) shows it reads process.env.FLOCI_ENDPOINT (Ho("aws")=> process.env.FLOCI_ENDPOINT ?? "http://localhost:4566") to reach the floci emulator's AWS endpoint, plus process.env.PORT. Set env FLOCI_ENDPOINT=http://floci.floci.svc.cluster.local:4566 exactly as the row mandates (A10's floci Service DNS in the shared ns + port 4566). No env name deviation — the row was accurate. (5) THE A10 SERVICE-LINK LESSON — carried forward PROACTIVELY: enableServiceLinks: false set on the pod template BEFORE any crash. Rationale: in the shared `floci` ns Kubernetes would inject legacy Docker service-link env for every Service — FLOCI_PORT (A10's "floci" Service) AND FLOCI_UI_PORT (this pack's own "floci-ui" Service), both tcp://<clusterIP>:<port>; and the floci-ui server reads a whole FLOCI_* family (FLOCI_ENDPOINT, FLOCI_DEFAULT_ACCOUNT_ID, FLOCI_GCP_ENDPOINT, …), so an injected FLOCI_*=tcp://… is exactly the collision class that CrashLoopBackOff'd floci in A10. With the flag set, floci-ui came up clean on the FIRST conformance run — no crash, deploy Available=True (1/1) at ~27s (see REVIEW). (6) EXPOSE = A7/A10 pattern (urls only, NO authSecretRef): pack.cue expose.urls=["https://floci-ui.${GATEWAY_HOST}"] (D11 record) + manifests/30-httproute.yaml — an HTTPRoute "floci-ui" in ns floci, parentRef Gateway cube-idp in ns ${GATEWAY_PACK}, hostname floci-ui.${GATEWAY_FQDN}, backend Service floci-ui port 4500, every schema-defaulted field written explicitly (argocd SSA-diff rule). floci-ui authenticates to the emulator with the same any/dummy AWS creds floci accepts → no bootstrap Secret → no authSecretRef/impliedFields. (7) #Values — OMITTED (no chart; authored manifests). (8) CONFORMANCE EXTRA-PACK: used the harness's existing CUBE_IDP_CONFORMANCE_EXTRA_PACK_DIR (on $PACKS main from A3) pointing at the PUBLISHED public oci://ghcr.io/cube-idp/packs/floci:0.1.0 (verified pullable) so floci is delivered before floci-ui — no COPY needed, no conformance.sh change. (9) CRD-ORDERING RACE (A10's owner-accepted harness class): floci-ui ships an HTTPRoute and the floci EXTRA_PACK ships one too, so the race was possible — but it did NOT bite on EITHER of my two green runs (the gateway created the HTTPRoute CRD before the non-gateway packs first reconciled; log shows "[gateway-crd] Gateway API HTTPRoute CRD established" before "[health] waiting"), so NO flux-reconcile nudge was needed. Recorded as observed-not-hit; the A10 harness follow-up recommendation stands unchanged. A tasks mint no CUBE codes; $ROOT code untouched (ledger only; go.mod gains no module; no $ROOT Go gate/fences due — data-only $PACKS pack). Three untracked docs/superpowers drafts (cluster-forprovider*, kind-config-reference) + an unrelated ` M README.md`/`docs/plugin-use-cases.md` in $ROOT belong to other sessions — NEVER added/edited/touched; every ledger commit used targeted `git add`/`git commit <plan-file>` only.  REVIEW: pre-conformance render validation: `cube-idp pack push packs/floci-ui oci://127.0.0.1:1/nope` reached the layer-push step and failed ONLY on the dead network (CUBE-4015) — proves pack.cue + expose block compiled and all manifest docs (20-floci-ui with Deployment+Service, 30-httproute) parsed + the artifact layer built. LIVE conformance GREEN — ran TWICE from the worktree (env clean before each: kind get clusters empty, 18443 free, docker 29.4.0, no stray floci-ui containers; gateway defaulted to oci://ghcr.io/cube-idp/packs/traefik:0.2.0, EXTRA_PACK=oci://ghcr.io/cube-idp/packs/floci:0.1.0): `CUBE_IDP_E2E_GATEWAY_PORT=18443 CUBE_IDP_CONFORMANCE_EXTRA_PACK_DIR=oci://ghcr.io/cube-idp/packs/floci:0.1.0 bash hack/conformance.sh floci-ui /tmp/cube-idp-a11` → up delivered traefik@0.2.0 + floci@0.1.0 + floci-ui@0.1.0, cube.lock (3 packs), "[health] 3 component(s) ready", Access lines "floci https://floci.cube-idp.localtest.me:18443" + "floci-ui https://floci-ui.cube-idp.localtest.me:18443", one-shot status ✔ cube-idp-floci Ready ✔ cube-idp-floci-ui Ready ✔ cube-idp-traefik Ready (38 objects), "CONFORMANT: floci-ui", cluster torn down (both runs; kind get clusters empty + 18443 FREE after each). Step 4 health-gate evidence captured DURING run 2 via an in-run poller (kubectl --context kind-conf-flociui get deploy -n floci): floci-ui progressed 0/1 Available=False (2s) → 1/1 Available=True at 27s; `deployment.apps/floci-ui 1/1 1 1` Available=True running floci/floci-ui:0.2.0@sha256:03a26114… (NO CrashLoopBackOff — the enableServiceLinks:false fix held), pod floci-ui-5f446857c6-cpzjz 1/1 Running 0 restarts. The engine reports the pack Ready only when its <health> Deployment (floci-ui) is Available, so the green status --exit-status is definitive proof floci-ui reached Available — exactly the A11 row's `deploy floci-ui Available` gate. Task-level Go gate N/A ($PACKS is data-only; no cmd/ or internal/ touched). OWNER GATE (standing pre-authorization §5) RAN by me: `git -C $PACKS tag floci-ui/v0.1.0 96769e2 && git -C $PACKS push origin floci-ui/v0.1.0` (ONE tag per push → "* [new tag]") → publish run 29663777208 → conclusion SUCCESS (watched to green); `gh attestation verify oci://ghcr.io/cube-idp/packs/floci-ui:0.1.0 --owner cube-idp` → exit 0, 1 attestation, predicateType https://slsa.dev/provenance/v1 (keyless GitHub-native, GT10), subject ghcr.io/cube-idp/packs/floci-ui @ sha256:a9007f8926848de3…, signer publish.yml@refs/tags/floci-ui/v0.1.0; `gh api orgs/cube-idp/packages/container/packs%2Ffloci-ui` → visibility "public" (version_count 3; org default is public — A2/A9/A10 precedent; recorded, NOT flipped). Shared template checkboxes (Steps 1-5 ticked by A1, Step 6 owner box unticked) left EXACTLY as-is — A11 is the LAST A task but Step 6 stays a shared checkbox; not ticked for others.  BLOCKERS: none  HANDOFF: floci-ui pack live on $PACKS main at 96769e2, published as oci://ghcr.io/cube-idp/packs/floci-ui:0.1.0 (public, attested). Branch p5/a11-floci-ui kept, worktree removed. A11 is the LAST Wave A task — all of A1–A11 are now DONE/DONE_WITH_CONCERNS and published. floci-ui is a console FOR floci (A10): it shares ns `floci`, ships no Namespace object, and needs the floci pack present at up-time (conformance uses CUBE_IDP_CONFORMANCE_EXTRA_PACK_DIR=oci://ghcr.io/cube-idp/packs/floci:0.1.0). VERIFY-API corrections captured for any re-pin: floci-ui serves ONE port 4500 (no 4501 API port — image sets PORT=4500), health = GET / (no /health path), backend env = FLOCI_ENDPOINT (confirmed correct). The A10 CRD-ordering-race harness follow-up remains an open owner/harness item (not hit on A11's runs, but structurally still possible for any HTTPRoute-bearing pack). Remaining Phase 5 open tasks: P10 (plugin install) then F1 (CLI coherence, LAST) — neither is a Lane A pack.
```
