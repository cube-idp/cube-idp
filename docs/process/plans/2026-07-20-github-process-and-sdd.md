...
| T11 | Pilot: issue #7 → ADR-0043 (ADR track) | T2,T5,T9 | **yes** | DONE |
...
| T14 | Instantiate the Projects v2 board per ADR-0042 §Board | T5,T13 | **yes** | DONE (owner, 2026-07-22) |
...
- [x] **Step 4: Write `epic.yml`** (ADR-track tracker)
...
- Follow-ups: #17–#20 must each get a ADR-track revive or a reasoned close.
...
#### T14 Outcome
- STATUS: DONE (owner-executed 2026-07-22, after PR #22 merged)
- BRANCH: n/a — board instantiation is outward GitHub/UI config, not repo commits. Ledger recorded via PR `process/retire-track-letters`.
- COMMITS: n/a (no repo artifacts; board + org config only)
- FINDINGS:
  - Board created: **cube-idp delivery**, org project **#2**, https://github.com/orgs/cube-idp/projects/2.
  - Status single-select set byte-exact to `Backlog · Proposed · Accepted · In progress · In review · Done` via `updateProjectV2Field` GraphQL (the plan's UI-or-GraphQL alternative — used GraphQL, no UI needed for options). `Estimate` NUMBER field created via `gh project field-create`. `Iteration` field NOT created (no CLI/GraphQL create path; optional, `board-sync` never reads it) — left for optional UI.
  - Built-in workflows (UI, no write API): Auto-add `cube-idp/cube-idp` `is:issue is:open` = enabled; Item added → Backlog; Item closed → Done; Item reopened → In progress; PR-related built-ins disabled. Verified enabled-state via `projectV2.workflows` GraphQL.
  - Credential: org GitHub App `cube-idp-board-bot` (Projects: read&write, Issues: read) installed. Org config set (owner, UI): variable `BOARD_APP_ID=4363187`, variable `BOARD_PROJECT_NUMBER=2`, secret `BOARD_APP_PRIVATE_KEY`. Confirmed via `gh variable list --org` / `gh secret list --org`.
  - **Auto-add cold-start lag:** the first scratch issue (#34) did NOT auto-add within 60s (workflow was freshly enabled); a second issue (#35) auto-added in ~10s once warm. Not a defect — GitHub auto-add is async and slow immediately after enabling. Auto-add is also non-retroactive: pre-existing open issues are not swept onto the board; only issues opened/modified after enablement.
- REVIEW: end-to-end smoke test PASSED — #35 auto-added → Backlog; #34 manual-add → Backlog; both closed → Done; all Status transitions occurred with zero manual edits. Scratch issues #34/#35 closed (sit in Done).
- BLOCKERS: none.
- HANDOFF: `board-sync.yaml`'s four middle transitions (Proposed/Accepted/In progress/In review) are wired and credentialed but **not yet exercised live** — they fire on the first real ADR PR (join key `[ADR-NNNN]`) or delivery PR (`Closes #N`). The ADR-0043 PR (owner follow-up) will be their first live run. `BOARD_PROJECT_NUMBER=2` is the join value; Status option names must stay byte-exact or `board-sync` no-ops the match.
...
- FINDINGS:
  ...
  - **`docs/outstanding-todos.md` deliberately left in place** (temporarily allowed by the doc-consistency closed-set: `ALLOWED="adr architecture reference process archive vhs outstanding-todos.md"`). **OWNER ACTION REQUIRED:** its items must be converted to issues under the new process (ADR-0042 ADR track/Direct track), then the file archived/removed and dropped from the CI ALLOWED list. Until then the closed-set check passes with it present.
