...
| No engine-coupled type or URI scheme appears in the user-authored config schema: `spec.engine` exposes only `type`, `ref`, `values` and `selfManage`. | `internal/config/schema.cue` |
| The cluster's record of truth is the engine's own resources plus an SSA inventory and the inert `Pack` CRD (established but never reconciled by the CLI). | `internal/apply/inventory.go`, `internal/pack/manifests/pack-crd.yaml`, `internal/up/up.go` |
| The engine's own install left the interface: no `Install`/`InstallManifests` method remains on the seam. | `internal/engine/engine.go` |
| Removing engine tuning is backed by an explicit migration guard rather than silent acceptance: a config that still sets `engine.tuning` errors with `CUBE-0012`. | `internal/config/load.go` |

### Verification

- [ ] `internal/engine/engine.go` declares `type Engine interface` with exactly the seven
...
- [ ] `internal/engine/contract/contract.go` opens the `health_tolerates_fresh_cluster`
...
- [ ] `internal/engine/factory/factory.go` is `func New(spec config.EngineSpec) (engine.Engine, error)`
      switching on `spec.Type` over `flux` and `argocd`, erroring `CUBE-3001` otherwise, and
      reading no other field of `spec`.
- [ ] `internal/engine/engine.go` declares `const SelfArtifactName = "cube-engine"`, and
      `internal/up/up.go` declares `const engineSelfTag = "latest"`.
...
- [ ] `internal/engine/tune.go` does not exist; grepping `internal/engine/` for `embed.FS` and
      ...
      `InstallManifests`.
- [ ] `internal/config/load.go` emits `CUBE-0012` when a config still sets
      `engine.tuning`, and `internal/config/schema.cue` carries no `tuning` key.
