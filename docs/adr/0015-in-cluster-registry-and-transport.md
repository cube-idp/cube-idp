...
remote clusters) without assuming any ingress, LoadBalancer or node-port topology.
...
comprising at minimum a Namespace, a Deployment and a Service. Gitea is used for git
repository delivery only and never as a container registry.

Host-to-registry access is a **port-forward to the zot Service for all providers**. The
forward binds an ephemeral free local port to zot's port 5000 and returns a `127.0.0.1`
address plus a stop function the caller must `defer`.
...
`github.com/hashicorp/go-getter` via a go.mod `replace` directive and never fetched by
`go get` on the fork path, with cube-idp's own archive-extraction guards in front of it.
...
* Good, because port-forwarding works identically on every provider, so no provider
  needs a bespoke registry-exposure story.
...
* Bad, because a port-forward is a live process: the caller must own its lifetime, and
  a missed `defer stop()` leaks a goroutine and a local port.
...
| --- | --- |
| The in-cluster OCI registry is zot at the fixed address `zot.cube-idp-system.svc.cluster.local:5000`, with manifests embedded in the binary and every object except the Namespace in `cube-idp-system`. | `internal/registry/zot.go:17`, `internal/registry/manifests/zot.yaml` |
| project-zot/zot is the in-cluster OCI registry package (replacing any use of Gitea as a container registry), with oras-go v2 used for artifact push. | `go.mod:26` |
| Host-to-registry access is a port-forward to the zot Service for all providers, binding an ephemeral local port to port 5000 and returning a `127.0.0.1` address plus a caller-deferred stop function. | `internal/registry/portforward.go:21-28`, `internal/kube/portforward.go:78` |
...
- [ ] `internal/registry/manifests/zot.yaml` contains exactly a Namespace `cube-idp-system`, a Deployment `zot` and a Service `zot`, the latter two both `namespace: cube-idp-system`.
...
- [ ] `internal/kube/portforward.go:78` returns a `127.0.0.1:<ephemeral>` address plus a stop closure; `internal/up/up.go:287-291` calls it and `defer stop()`s.
