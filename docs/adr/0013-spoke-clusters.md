...
| --- | --- |
| `spec.spokes` is a list of `{name, cluster}` entries whose names match `^[a-z0-9][a-z0-9-]{0,30}$`, enforced by the CUE schema (generic CUBE-0002). | `internal/config/schema.cue:42-43` |
| Spoke-name uniqueness within a cube, and `cluster.context` for provider `existing`, are enforced pre-CUE as the typed CUBE-8001. | `internal/config/load.go:252-276` |
| Spokes support only providers `kind` and `existing`; a k3d spoke is rejected with CUBE-8001 while a k3d hub stays supported. | `internal/config/schema.cue:45`, `internal/config/load.go:252-276` |
| Spokes are declarative `cube.yaml` content; `spoke add` only declares and `spoke remove` only undeclares, with bootstrap, registration and pruning deferred to the next `up`. | `cmd/spoke.go:44-53` (add), `cmd/spoke.go:152-158` (remove) |
| Spoke bootstrap creates namespace `cube-idp-system`, ServiceAccount `cube-idp-<engine>`, and ClusterRoleBinding `cube-idp-<engine>-admin` bound to `cluster-admin`. | `internal/spoke/bootstrap.go:23,43-63` |
| Spoke bootstrap is idempotent via server-side apply; bootstrap failures are CUBE-8002 and token-issuance failures are CUBE-8003. | `internal/spoke/bootstrap.go:74-89` |
| Spoke service-account tokens are minted via TokenRequest with a 10-year lifetime. | `internal/spoke/bootstrap.go:26,83-85` |
| Cube-created spoke clusters are named `<cube>-spoke-<name>`; an `existing` spoke keeps its own name. | `internal/up/up.go:750-755` |
...
| Spokes support both flux and argocd engines from day one, each registered as a single hub Secret. | `internal/spoke/register.go:36-74` |
...
| A kind spoke's hub-facing API URL comes from kind's internal kubeconfig (`https://<cluster>-control-plane:6443`). | `internal/up/up.go:760-771` |
| For `existing` spokes the endpoint is the connection's own server URL and reachability is the operator's responsibility. | `internal/up/up.go:757-759,772` |
...
| `spoke remove` leaves a kind spoke's cluster running unless `--delete-cluster` is passed, which requires confirmation or `--yes` and refuses non-interactively with CUBE-0010. | `cmd/spoke.go:159-165,189-213` |
| `down` deletes cube-created kind spoke clusters best-effort after hub teardown unless `--keep-cluster` is passed, and leaves `existing` spokes untouched, printing the manual RBAC-removal commands. | `cmd/down.go:134-155` |
...
| A registered spoke appears in `status`, `spoke list`, the inventory record driving the `down` cascade, and doctor's checks. | `cmd/status.go:320-328`, `cmd/spoke.go:82-101`, `internal/up/up.go:724-726`, `internal/doctor/doctor.go:328-350` |
...
| `down --keep-cluster` keeps cube-created kind spoke clusters as well as the hub cluster. | `cmd/down.go:139-142` |
| CI proves engine parity for spoke registration: the e2e engine axis runs the kind-spoke registration test under both flux and argocd, which is also the parity evidence for engine pin bumps. | `.github/workflows/ci.yaml:27-29,54`, `tests/e2e/e2e_test.go:580-632` |

### Verification

- [ ] `internal/config/schema.cue:45` pins the spoke provider enum to `*"kind" | "existing"` and excludes k3d.
- [ ] `internal/config/load.go:252-276` returns `diag.CodeSpokeProviderUnsupported` (CUBE-8001) for a duplicate spoke name, an `existing` spoke without `cluster.context`, and any provider outside kind/existing — and does not check the name regex, which is CUE's job (`internal/config/schema.cue:43`).
...
- [ ] `.github/workflows/ci.yaml:27-29` defines a provider x engine end-to-end matrix covering both flux and argocd, and `tests/e2e/e2e_test.go:580-632` runs the kind-spoke registration test on every leg of it.
