...
cube-idp forces only the fields it owns — `kind`/`apiVersion`, gateway port mappings,
containerd registry and `certs.d` patches, the node image derived from `kubernetesVersion`,
and control-plane `extraMounts`. Every other field remains user-controlled and untyped by
cube-idp.
...
| Pin the gateway `extraPortMapping` containerPort to `config.GatewayNodePort` (30443), since a kind port mapping only reaches a NodePort Service when containerPort equals the Service nodePort. | `internal/cluster/kindp/merge.go:36,107-120` |
...
| Write registry mirror and insecure-registry settings via `containerdConfigPatches`, and bind the `certs.d` directory (hosts.toml/ca.crt) through `CertsD`. | `internal/cluster/kindp/merge.go:166-174,205-216` |
| Every kind field other than the ones cube-idp writes remains user-controlled through layer 2 `forProvider` and is not typed by cube-idp. | `internal/cluster/kindp/merge.go:73-84` |
| `config render-cluster` is pure, cluster-free and file-free (zero `CertsD` / zero `ZotMirror`), creates no trust material, and fails with CUBE-0004 on a provider that creates no cluster. | `cmd/config.go:44` |
...
- [ ] `internal/cluster/kindp/merge.go:205-216` emits containerd mirror and `insecure_skip_verify` patches; `merge.go:168-174` adds the `config_path = "/etc/containerd/certs.d"` patch plus the `CertsD` extraMount.
...
a hard migration error, CUBE-0011: `internal/config/load.go:58-83` probes for it before CUE
