...
| --- | --- |
| cube-idp never modifies the OS trust store implicitly; trust-store changes happen only inside the explicitly-consented `cube-idp trust` command and are fully reverted by `cube-idp down`, while `up` may generate a local CA but never touches the OS store. | `cmd/trust.go`; `cmd/trust.go`; `cmd/down.go`; `internal/up/up.go` |
...
- [ ] `internal/trust/ca.go` — the package doc states nothing in `internal/trust`
      touches the OS trust store implicitly.
- [ ] `internal/up/up.go` — `up` calls only `trust.Dir` + `trust.EnsureCA`. Run
      `grep -rn 'InstallOS\|truststore' internal/up cmd/up.go` — it must return nothing.
- [ ] `cmd/trust.go` — `trustInstall`/`trustUninstall` are seams bound to
      `trust.InstallOS`/`trust.UninstallOS`; `cmd/trust.go` gates install behind a
...
- [ ] `cmd/down.go` — `down` reads `trust.LoadState` and calls `trustUninstall`
      only when `st.Installed` is true.
- [ ] `internal/trust/ca.go` — `EnsureCA` returns the existing `ca.crt` if present,
      otherwise prefers `adoptMkcertCA` over generating a new key.
- [ ] `internal/trust/ca.go` — the CA key is written
      and chmod'd to `0600` on both the generate and adopt paths
      (`go test ./internal/trust/...`).
- [ ] `internal/up/tls.go` — `ensureGatewayTLS` reuses a live secret when
      `ca.LeafStillValid` reports >30 days left for both hosts, so repeated `up` runs
      see no churn.
- [ ] `internal/trust/ca.go` — `mkcertCAROOT()` resolves `$CAROOT` first, then the
      per-OS mkcert default (darwin / windows / XDG).
- [ ] `internal/trust/ca.go` — the `adoptMkcertCA` doc states adoption is
...
Note: `trust.Dir()` (`internal/trust/ca.go`) resolves `os.UserConfigDir()/cube-idp`
