...
`sync`) all need to report progress, and they need to report it in three very different
...
`ui.RunPipeline` owns a single command's pipeline — producer on a goroutine, renderer on
the calling goroutine — and guarantees on return that the terminal is fully released,
...
point via `ui.RenderError(err)` followed by a non-zero exit, never by a renderer. stderr
...
* Bad, because commands that stay on `ui.Printer` (`status`, `doctor`, `get`, `diff`,
  `upgrade --plan`, `init`) get none of these guarantees, leaving two output seams in
  the codebase.
...
| On failure the live renderer stops the live region, flushes in-flight state as permanent lines and exits before the diagnosis block renders, so a diagnosis can never be overwritten or trapped in an alt screen. | `internal/ui/render/live.go:140-144` |
| The Bubble Tea program's lifetime is strictly bounded by the command's `RunE` — it quits after the terminal event and no goroutine it started survives. | `internal/ui/pipeline.go:160-165` |
| `ui.RunPipeline` owns one command's pipeline, producer on a goroutine and renderer on the calling goroutine, returning only after the terminal is released and exactly the producer's error is available. | `internal/ui/pipeline.go:124-176` |
...
| *(superseded)* The live renderer covers `up` and `down` only; all other commands keep static output and request/response commands keep `ui.Printer` as their seam. | `internal/ui/pipeline.go:58-68` |
| *(superseded)* `RunPipeline` maps modes to renderers four ways: JSON → JSON, Live → Live, Styled → Live on a TTY else Plain, Plain → Plain. | `internal/ui/pipeline.go:78-110` |
...
(`internal/ui/pipeline.go:58-68`) extended the stream to `pack`, `plugin`, `repo`, `sync`
and `vendor`, and `status` gained its own inline Bubble Tea watch program
(`cmd/status.go:145`). The residual truth — that `status`, `doctor`, `get`, `diff`,
...
(`internal/ui/pipeline.go:96-110`) diverges further: `ModeStyled` on a real TTY gets
